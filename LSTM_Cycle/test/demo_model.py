# @Time    : 2020/3/17 上午 10:57
# @Author  : FRY--
# @FileName: demo_model.py
# @Software: PyCharm
...
class LSTM_Cycle_Test(nn.Module):
    def __init__(self, outclass=15, T=5):  # outclass 输出的通道数；T为区间间隔
        super(LSTM_Cycle_Test, self).__init__()
    ...
    def forward(self, images, heatmaps_start_end_gt, center_map):
        '''

        :param images: []
        :param heatmaps_start_end_gt: []
        :param center_map: []
        :return:
        '''
        # ------------------------------------------------------------------------------
        # Forward
        # test logical
        # forward  calculate 1 -> 2,3,4
        # backward calculate 5 -> 4,3,2
        # correspond images  calculate mean coordinates values
        # ------------------------------------------------------------------------------
        ...
        for j in range(self.T):
            img = images[j]  # 0,1,2,3,4
            heatmaps, cell, hide = self.frameNormal(img, heatmap_pre, center_map, cell, hide)
            heatmaps_forward.append(heatmaps)
            heatmap_pre = heatmaps

        # backword calculate
        back_list = [p for p in range(self.T)]
        back_list.reverse() # [4,3,2,1,0]
        for index in back_list:
            img = images[index]
            heatmaps, cell, hide = self.frameNormal(img,  heatmap_pre_right, center_map, cell_2, hide_2)
            heatmaps_backward.append(heatmaps)
            heatmap_pre_right = heatmaps
        heatmaps_backward.reverse()

        return heatmaps_forward, heatmaps_backward
